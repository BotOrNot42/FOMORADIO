...
class BaseCollector:
    ...
    def process(self) -> List[Dict]:
        ...
        """
        records, timestamp_key, content_key, user_key = self.fetch()
        granular_records = []
        for each_record in records:
            timestamp = each_record[timestamp_key]
            content = each_record[content_key]
            user = each_record[user_key]
            del each_record[timestamp_key]
            del each_record[content_key]
            del each_record[user_key]
            granular_records.append(
                Granule(self.source, timestamp, content, user, each_record)
            )
